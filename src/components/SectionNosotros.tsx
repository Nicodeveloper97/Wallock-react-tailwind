...
<section className="bg-[#F2F2F7] pt-12 pb-8 mb-0 flex flex-col items-center">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4 w-full max-w-6xl mx-auto">
    {skillData.map((service) => (
      <div
        key={service.name}
        className="font-quicksand text-center bg-[#F2F2F7] border-[#0A2FDA] w-full max-w-md h-auto p-4 md:p-6 lg:p-8 rounded-lg shadow-lg border border-solid opacity-100 hover:shadow-xl"
      >
        <div className="flex items-center justify-center mb-2 text-[#4361EE]">
          <div className="text-xl mr-2">{service.icon}</div>
          <h1 className="text-md font-semibold text-black">{service.name}</h1>
        </div>
        <p className="text-slate-500 text-sm lg:text-base">{service.description}</p>
      </div>
    ))}
  </div>
</div>
